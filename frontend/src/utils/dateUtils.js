...
export const formatGregorianDate = (dateStr) => {
  if (!dateStr) return '';
...
// פורמט תאריך לועזי מ-DD/MM/YYYY ל-YYYY-MM-DD
export const formatGregorianDateForInput = (dateStr) => {
  if (!dateStr) return '';
  
  // אם התאריך כבר בפורמט YYYY-MM-DD, להחזיר אותו כמו שהוא
  if (/^\d{4}-\d{2}-\d{2}$/.test(dateStr)) {
    return dateStr;
  }
  
  // אחרת, להמיר מפורמט DD/MM/YYYY
  const parts = dateStr.split('/');
  if (parts.length === 3) {
    return `${parts[2]}-${parts[1]}-${parts[0]}`;
  }
...
export const convertGregorianToHebrew = (gregorianDate) => {
...
export const convertHebrewToGregorian = (hebrewDate) => {
...
export const calculateDaysUntilNextBirthday = (birthDate, primaryDateFormat) => {
  try {
